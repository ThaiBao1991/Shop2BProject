...
import bao.code.shop2b.admin.customer.CustomerCsvExporter;
...
public class ShippingRateController {
	...
	moduleURL = "/shipping_rates") PagingAndSortingHelper helper, @PathVariable(name="pageNum") int pageNum ) {
	...
	public String saveRate(ShippingRate rate, RedirectAttributes ra) {
		...
		} catch (ShippingRateAlreadyExitsException e) {
	...
	public String updateCODSupport(@PathVariable(name = "id") Integer id,
		...
		try {
			...
			ra.addFlashAttribute("message","COD support for shipping rate ID " +id +" has been " +supported);
		} catch (ShippingRateNotFoundException e) {
			ra.addFlashAttribute("message",e.getMessage());
		}
		return defaultRedirectURL;
	...
	@GetMapping("/shipping_rates/delete/{id}")
		public String deleteRate(@PathVariable(name="id") Integer id,
			...
			try {
				service.delete(id);
				ra.addFlashAttribute("message", "The shipping rate ID " + id +" has been deteted.");
			} catch (ShippingRateNotFoundException e) {
				ra.addFlashAttribute("message", e.getMessage());
			}
			return defaultRedirectURL;
		}
	@GetMapping("/shipping_rates/export/csv")
	public void exportToCSV(HttpServletResponse response) throws IOException{
		List<ShippingRate> listShippingRates = service.listAllShippingRates();
		ShippingRateCsvExporter exporter = new ShippingRateCsvExporter();
		exporter.export(listShippingRates, response);
	}
}
