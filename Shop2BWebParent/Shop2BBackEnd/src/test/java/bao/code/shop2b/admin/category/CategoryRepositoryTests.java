...
public class CategoryRepositoryTests {
	...
	public void testCreateSubCategory() {
		...
		List<Category> addCategoryList = new ArrayList<Category>();
		addCategoryList.add(laptops);
		addCategoryList.add(components);
		
		repo.saveAll(Collections.unmodifiableList(addCategoryList));
		
		parent = new Category(2);
		Category Cameras= new Category("Cameras",parent);
		Category SmartPhones= new Category("SmartPhones",parent);
		repo.save(Cameras);
		repo.save(SmartPhones);
		
		parent = new Category(5);
		Category Mainboard= new Category("Mainboard",parent);
		repo.save(Mainboard);
		assertThat(savedCategory.getId()).isGreaterThan(0);
	}
